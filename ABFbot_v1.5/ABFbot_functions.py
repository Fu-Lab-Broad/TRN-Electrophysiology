...
def get_index_sharp_increase(array, start_index, end_index=None, threshold=0, num_neighbors=100):
    ...
    for index in range(start_index, end_index):
        neighbor_after_indicies = index + np.array(list(range(1, num_neighbors+1)))
        if (neighbor_after_indicies >= len(array)).any():
            neighbor_after_indicies = np.array(list(range(index, len(array))))
        difference = array[neighbor_after_indicies] - array[index]
...
def get_burst_details(df_peak_info, burst_index, time, voltage, voltage_derivative, current):
    ...
    ## Nolan's way to calculate ahp
    # _,_,resting_membrane_potential = get_resting_membrane_potential(voltage, current)
    # ahp = resting_membrane_potential - voltage[i_post_repolar_bottom]

    ## Megan's way to calculate ahp
    if i_post_repolar_bottom is not None:
